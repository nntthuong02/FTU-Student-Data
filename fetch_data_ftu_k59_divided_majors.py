...
#Sua:
# K56, 57, 58
# URL = "https://tracuuthongtin.ftu.edu.vn/08_select_tracuuemail_msv2020.php"
# K59
...
CD_VALUES = ["08","09", "10", "11", "12", "13", "14", "15", "16", "17", "18"]
#Sua: Ten khoa
RESULT_DIR = "data/k59"
os.makedirs(RESULT_DIR, exist_ok=True)

#Sua: K59 = 20, K58 = 19, K57 = 18, K56 = 17
def generate_msv_list(major_values):
    return [
...
async def process_data(major_values, result_file, error_file):
    errors = []
    msv_list = generate_msv_list(major_values)
...
async def main():
    ...
    for department, majors in departments.items():
        #Sua: Doi ten file Data va errors cho dung khoa
        result_file = os.path.join(RESULT_DIR, f"DataFtu_k59_{department.replace(' ', '_')}.csv")
        error_file = os.path.join(RESULT_DIR, f"errors_k59_{department.replace(' ', '_')}.csv")
        tasks.append(process_data(majors, result_file, error_file))
