...
# Danh sách năm (thay thế 720)
#Sua
MAJOR_IDS = ["720", "770"]

# Tạo danh sách MSSV từ CD_VALUES và MAJOR_IDS
#Sua: K59 = 20, K58 = 19, K57 = 18, K56 = 17
msv_list = [f"20{cd}{major}{str(i).zfill(3)}" for cd in CD_VALUES for major in MAJOR_IDS for i in range(1000)]
...
#Sua
RESULT_FILE = os.path.join(RESULT_DIR, "DataFtu_k59.tttm.csv")
...
async def process_data():
    ...
    if errors:
        #Sua
        error_filename = os.path.join(RESULT_DIR, "errors.tttm.csv")
        with open(error_filename, "w", encoding="utf-8", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(["Mã sinh viên lỗi"])
            for err in errors:
                writer.writerow([err])
        print(f"⚠️ Đã lưu {len(errors)} lỗi vào {error_filename}")
